...
## [2026-05-15] 예약 모니터링 실패 — `/onlineRsv/list` 직접 진입 오류
- **증상**: 실제 브라우저에는 달력이 표시되지만 봇 로그는 `showCalendar`/달력 요소 대기 타임아웃 후 `가능 []`으로 판정.
- **원인**: 사이트가 예약 달력을 `/main` 위젯에서 렌더링하도록 바뀌었고, 기존 봇이 직접 진입하던 `/onlineRsv/list`는 `프로그램 오류가 발생하였습니다` 페이지를 반환. 또한 연수원 선택 UI가 `select option`이 아니라 `#ser_yeonsu_gbn_ul li` + hidden input 구조로 바뀜.
- **수정**: 모니터링/예약 시작 진입점을 `/main`으로 변경하고, 연수원 선택을 실제 UI의 `li` 클릭 방식으로 변경. 달력 준비 대기를 `rsv_day_init`/`td.targetDate[data-date]` 기준으로 정리. 객실 목록 이동 후 사이트 오류 페이지를 감지하면 명확한 `BookingError`로 처리.
- **검증**: `서천연수원 2026-05-26` 기준 `check()`가 `['20260526']` 반환.
- **파일**: `checker.py`

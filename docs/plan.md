...
├── checker.py               ← Chromium 폴백 + noRsvClose 안전 호출
├── scheduler.py             ← 변경 없음
├── notifier.py              ← 변경 없음
...
# Linux에서는 channel 유무와 무관하게 --no-sandbox 적용
if platform.system() == "Linux":
    launch_kwargs["args"] = ["--no-sandbox", "--disable-dev-shm-usage"]

# 기관배정 팝업: noRsvClose() 미정의 시 직접 DOM 숨김 폴백
page.evaluate("""() => {
    if (typeof noRsvClose === 'function') noRsvClose();
    else { /* 직접 팝업 요소 숨김 */ }
}""")
```
